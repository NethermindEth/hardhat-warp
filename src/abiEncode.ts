...
export function abiEncode(
  ...
  return "0x" + tp.flatMap(ty => abiEncode_(ty, stream)).join("");
}

export function abiEncode_(tp: ParamType, values: IterableIterator<SolValue>): string[] {
  if (tp.baseType.startsWith("uint") || tp.baseType.startsWith("address")) {
    ...
    const padded = mod == 0 ? encoded : encoded.padEnd(Math.floor(encoded.length / 64) + 1, "0")
    return [padded];
